...
class FineTuningDatasetWithNegativesDis(Dataset):
    def __init__(self, args, transform=None):
        self.args = args
        data = torch.load(configs.finetuning_dataset_path)
        self.transform = transform
    ...
    def __getitem__(self, idx):
        ...
        subject = annos["sop_data"]["sop_decomposition"]["subject"]
        if str(subject) == "None":
            subject = "None"
        subject = self.text_transform([subject])

        negated_object = annos["sop_data"]["sop_chosen_object"]
        negated_object = self.text_transform([negated_object])

        return image, negative_image, caption, negative_prompt, subject, negated_object
