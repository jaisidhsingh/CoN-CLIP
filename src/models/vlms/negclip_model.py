...
class NegClipRunner():
	def __init__(self, model_name):
		...
		self.ckpt_path = f"/workspace/jaisidh/btp/checkpoints/pretrained_vlms/negclip_b32.pth"
	...
	def score_over_loader(self, loader):
		...
		# log results	
		tqdm.write(f"Accuracy of NegCLIP {self.model_name}: {accuracy}")
	...
	def score_over_loader2(self, loader):
		...
		# log results
		tqdm.write(f"Retrieval Accuracy of NegCLIP [**for Positives**]{self.model_name}: {pos_accuracy}")
		tqdm.write(f"Retrieval Accuracy of NegCLIP [**for Negatives**]{self.model_name}: {neg_accuracy}")
