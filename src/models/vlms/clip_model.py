...
class ClipRunner():
	...
	def score_over_loader(self, loader):
		...
		for idx, (images, captions, negative_captions) in enumerate(loader):
			...
			preds_combined = torch.cat([preds1, preds2], dim=1).to(self.device) # shape: Bx2
		...
		# log results	
		tqdm.write(f"Accuracy of CLIP {self.model_name}: {accuracy}")
	...
	def score_over_loader2(self, loader):
		...
		# log results
		tqdm.write(f"Retrieval Accuracy of CLIP [**for Positives**]{self.model_name}: {pos_accuracy}")
		tqdm.write(f"Retrieval Accuracy of CLIP [**for Negatives**]{self.model_name}: {neg_accuracy}")
